...
Usage:
    ...
    python train.py --skip_phase1               # skip Phase 1, start from Phase 2
...
def main():
    ...
    parser.add_argument("--skip_phase1", action="store_true",
                        help="Skip Phase 1 and directly start Phase 2 (fine-tuning)")
    ...
    if not args.skip_phase1:
        ...
        # 跳过 Phase 1：直接加载 Phase 1 保存的最佳模型
        phase1_ckpt = os.path.join(config.MODEL_DIR, "best_phase1.pth")
        if not os.path.isfile(phase1_ckpt):
            raise FileNotFoundError(
                f"Cannot skip Phase 1: {phase1_ckpt} not found. "
                "Please run Phase 1 first (without --skip_phase1)."
            )
        print(f"\nLoading Phase 1 checkpoint from {phase1_ckpt}")
        model.load_state_dict(torch.load(phase1_ckpt, map_location=device))
        best_acc = 0.0   # Phase 2 的最佳精度重新开始记录
    ...
    # Phase 2 使用更小的学习率，调度器的 T_max 为 Phase 2 的 epoch 数
